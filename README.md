# Math Gone Too Far

**Version**: 0.2.0

## Project Status

Early development. Features may or may not exist or be incomplete.

## Licence

The source code in `src/` is licensed under the GNU General Public Licence v3.0. See the [LICENCE](./LICENCE) for the full licence text.

Fonts in `assets/fonts` are sourced from [Google Fonts](https://fonts.google.com) and are available under the [SIL Open Font License v1.1](https://openfontlicense.org/open-font-license-official-text/).
